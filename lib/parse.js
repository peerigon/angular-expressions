...
ASTCompiler.prototype = {
	compile: function (expression) {
		...
		const fs = require("fs");
		fs.writeFileSync("/tmp/code.js", fnString);
		// Then run prettier /tmp/code.js
	...
	recurse: function (
		...
		switch (ast.type) {
			...
			case AST.UnaryExpression:
				...
				break;
			case AST.BinaryExpression:
				console.log(JSON.stringify({"intoId": intoId}));
			...
			case AST.MemberExpression:
				...
				self.recurse(
					...
					function () {
						...
						console.log(JSON.stringify({"left,right": left,right}));
								if (ast.computed) {
									if (ast.property.type !== AST.Literal) {
										self.recurse(ast.property, right);
									}
